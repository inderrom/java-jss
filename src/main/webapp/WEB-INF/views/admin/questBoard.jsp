...
$(function(){
	...
	$('.btn-tool[title="Remove"]').click(function() {
		...
		if(isDel > 0){
			alert("삭제되었습니다.")
			location.href = "/admin/boardDelete?boardNo="+boardNo+"&boardType=BRDCL0004";
		}
	});

	$("#createNotice").on("click",function(){
		location.href = "/admin/createBoard?boardType=BRDCL0004";
	});
});
						...
						<div class="card-body">
							<form>
								<div class="row">
									...
									<div class="col-md-2">
										<button type="button" class="btn btn-block btn-outline-info" id="createNotice">문의사항 등록</button>
									</div>
								</div>
							</form>
						</div>
						...
						<c:if test='${data.total != 0 }'>
							<ul class="pagination justify-content-center">
								...
								<c:if test='${data.startPage lt 11 }'>disabled</c:if>"
									...
									<a href="/admin/questBoard?currentPage=${data.startPage-10}&keyword=${param.keyword}&searchType=${param.searchType}"
								...
								<c:forEach var="pNo" begin="${data.startPage}" end="${data.endPage}">
									<li class="paginate_button page-item <c:if test='${currentPage eq pNo}'> active</c:if>">
										<a href="/admin/questBoard?currentPage=${pNo}&keyword=${param.keyword}&searchType=${param.searchType}"
