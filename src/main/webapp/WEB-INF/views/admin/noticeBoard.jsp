...
$(function(){
	...
	$('.btn-tool[title="Remove"]').click(function() {
		...
		if(isDel > 0){
			Swal.fire("삭제되었습니다.")
			location.href = "/admin/boardDelete?boardNo="+boardNo+"&boardType=BRDCL0001";
		}
	});

	$("#createNotice").on("click",function(){
		location.href = "/admin/createBoard?boardType=BRDCL0001";
	});
	...
	<div class="card">
		<div class="card-header">
			<div class="card-12">
				<div class="card">
					<div class="card-header">
						<h3 class="card-title">공지사항 글 목록</h3>
					</div>
					<div class="card-body">
						<div class="card-body">
							<form>
								<div class="row">
									...
									<div class="col-2">
										<button type="button" class="btn btn-block btn-outline-info" id="createNotice">공지사항 등록</button>
									</div>
								</div>
							</form>
						</div>
						<div class="row">
							<table class="table table-head-fixed text-nowrap">
								...
								<tbody>
									<c:forEach var="boardVo" items="${data.content}" varStatus="stat">
										<tr style="text-align: center;">
											<td>${boardVo.rnum}</td>
											<td>${boardVo.memId}</td>
											<td><a type="button" class="boardBtn" id="board-${boardVo.boardNo}">${boardVo.boardTitle}</a></td>
											<td><fmt:formatDate value="${boardVo.boardRegDt}" pattern="yyyy-MM-dd"/></td>
											<td>
												${boardVo.boardInqCnt}
						...
						<c:if test='${data.total != 0 }'>
							<ul class="pagination justify-content-center">
								...
								<c:if test='${data.startPage lt 11 }'>disabled</c:if>"
									...
									<a href="/admin/noticeBoard?currentPage=${data.startPage-10}&keyword=${param.keyword}&searchType=${param.searchType}"
								...
								<c:forEach var="pNo" begin="${data.startPage}" end="${data.endPage}">
									<li class="paginate_button page-item <c:if test='${currentPage eq pNo}'> active</c:if>">
										<a href="/admin/noticeBoard?currentPage=${pNo}&keyword=${param.keyword}&searchType=${param.searchType}"
